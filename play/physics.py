...
from .utils import clamp, is_circle, is_line
...
class Physics(object):
    ...
    def _make_pymunk(self):
        ...
        if not self.can_move and is_line(self.sprite):
            ...
            self._pymunk_body.angle = math.radians(self.sprite.angle)
            ...
            if is_circle(self.sprite):
                ...
                self._pymunk_shape = pymunk.Poly.create_box(
                    self._pymunk_body, (self.sprite.width, self.sprite.height))

        self._pymunk_shape.elasticity = clamp(self.bounciness, 0, .99)
    ...
    def bounciness(self, _bounciness):
        ...
        self._pymunk_shape.elasticity = clamp(self._bounciness, 0, .99)
    ...
    def obeys_gravity(self, _obeys_gravity):
        ...
        if _obeys_gravity:
            self._pymunk_body.velocity_func = pymunk.Body.update_velocity
        else:
            self._pymunk_body.velocity_func = lambda body, gravity, damping, dt: None